...
export class SelectTimingComponent implements OnInit {
  ...
  async displayAvailableSlots() {
    if (!this.STAFF_AVAILABLE_SLOT || this.STAFF_AVAILABLE_SLOT.length === 0) return;

    // No need to filter or process the data further
    this.DISPLAY_LIST = this.STAFF_AVAILABLE_SLOT.map(slotData => {
      return {
        ...
        shift_list: slotData.availableSlots.map((time, index) => ({
          ...
          time: time,
          is_active: false,
          is_disabled: false, // You can implement logic to disable past times if needed
        })),
      };
    });
  }
